...
def husbandry_system_context(request):
    ...
    husbandry_systems = HusbandrySystem.objects.values_list(
        'name',
        flat=True
    ).distinct()

    selected_husbandry_system = request.session.get(
        'husbandry_system', ''
    )

    # Store the selected husbandry so
    # that it persists as long as the current session lasts!
